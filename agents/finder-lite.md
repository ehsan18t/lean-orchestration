---
name: finder-lite
description: Low-effort variant of finder for the lean-orchestration "find" dispatch. Reviews an assigned slice of a diff or module through one named lens — or a caller-listed lens set — (code-defect, spec-conformance, design-critique) and returns deduped, anchor-backed findings labelled with severity and confidence. Same contract as finder, run at low reasoning effort. Use for breadth, where reading the slice carefully is the work. Use full finder where the defect class needs sustained reasoning: concurrency, lifecycle, caching, security, or spec drift across several files. Do not use to answer navigation questions (that is a navigator), to verify findings (that is a skeptic), or to edit files.
