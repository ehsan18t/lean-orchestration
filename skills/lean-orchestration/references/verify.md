...
**When it runs.** The review buys independence, which is worth paying for exactly where the author's blind spot is likely:

- **feature** route: always.
- **fixes** route: only when the diff touches a seam, a shared type or signature, more than one module, a user-visible number, persisted state, or will be auto-applied. Below that line the review is skipped on cost, not on coverage: gates plus the checklist walk are the whole verification, so the walk must also read the diff hunk by hunk and treat any hunk that serves no checklist item (a copied block, a guard removed "while here") as scope creep to revert or to record as a new item.
- **refactor** route: the behavior-preservation skeptic above is the review; do not also run a finder.
- **amend** route: off by default (see `amend.md`).
