...
- a **scope extension**: a new checklist item. A request that picks up an item marked `(deferred YYYY-MM-DD)` reopens that item instead of adding one, removing its deferred tag.
...
**A5 — Verify.** Gates first. Then re-walk **only the affected checklist items** plus any item that shares a file with the change, reporting each as met / not met. An item still tagged deferred is not re-walked. Then a reader who did not write the change, over the diff pinned to a fixed point, with the lenses named: code-defect, spec-conformance against the ledger items you paste in, and standards. Exempt only a change to a literal value, a copy string, or the ledger alone. One finder, naming the defect class in the dispatch when it needs sustained reasoning (concurrency, lifecycle, caching, security, a spec claim that resolves across files). A change touching persisted state, money, security, or a user-visible number adds a second reader on the `spec-conformance` lens alone (roles for both by the tier table, `SKILL.md` cost model item 2), checklist first, diff second; dedup the two returns by root cause, keep the lens labels, never re-rank across readers. Findings are dispositioned before delivery: a code-defect finding is fixed, or refuted with the anchor and logged; a spec finding rewrites the affected item; a standards finding is the user's call, with your recommendation: fix now, later (a new checklist item marked `(deferred YYYY-MM-DD)`), or as-is (a `Won't fix` entry copied to `wont-fix.md`), and with no one to answer your recommendation is applied and recorded as an assumption; a finding that touches data, security, or a wedge takes the skeptic route in the tier table (`SKILL.md` cost model item 2), ending at `skeptic-max`, before any fix is applied. A named defect fixed here gets its regression test seen red once before green counts.
