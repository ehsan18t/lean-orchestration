...
**A5 — Verify.** Gates first. Then re-walk **only the affected checklist items** plus any item that shares a file with the change, reporting each as met / not met. No review dispatch by default. Spend one `finder` (lenses: code-defect, spec-conformance against the ledger) only if the diff touches a seam, a shared type or signature, a user-visible number, or persisted state. A named defect fixed here gets its regression test seen red once before green counts.
