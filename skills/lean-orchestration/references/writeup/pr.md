...
**Effects and scope.** Only what someone must act on: a migration, a config change, a moved shared type, a behavior change a consumer depends on. Nothing here otherwise, and the section is dropped. Despite the label it is not the task's Scope and effects, which describes the whole blast radius; here it is only the part that requires an action.
