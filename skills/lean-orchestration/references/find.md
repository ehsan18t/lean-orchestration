...
- **One `finder` per slice** (role by the tier table, `SKILL.md` cost model item 2). Where the defect class needs sustained reasoning (concurrency, lifecycle, caching, security, spec drift across files), name it in the dispatch. Never plan a second sweep over a slice already read: it doubles the dispatch count the budget exists to hold down.
