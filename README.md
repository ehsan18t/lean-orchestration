...
The implementation review follows the same rule. It is one finder over the diff, reading it through three lenses at once (defects, conformance to the ledger's checklist, and the repo's own standards). It always runs on a feature. On a fix it runs only when the diff touches a seam, a shared type, more than one module, a user-visible number, or persisted state; a single-module fix that passed its skeptic and its gates has already been read by a fresh context once.
